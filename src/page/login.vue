    ...
    <div class="content">
      ...
      <div class="loginBox" ref="loginBox">
        ...
        <div class="login" v-if="!forgetpag">
          ...
          <div class="loginBtn" @click="outlogin">注册</div>
          <div class="join">
            <span>还没加入有爱教育？</span>
            <a href="#">[立即加入]</a>
          </div>
  ...
  methods: {
    ...
    login() {
      ...
      if (this.username == "") {
        ...
        this.$store.dispatch('administratorLogin', params).then((response) => {
    ...
    outlogin() {
      ...
      params.append('password', this.password);
      params.append('username', this.username);
      this.$store.dispatch('adminRegister', params).then((response) => {
        let res = response.data;
        if (response.statusText === 'OK' && response.status === 200) {
          console.log(res)
          this.$message(res)
